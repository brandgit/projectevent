...
export default function Festivales() {
  ...
  const filteList = eventData?.filter((event) => event.type === "festival")
  ...
  return (
    <div className='wrapperEvent'> 
     <div className='p-[80px]'>
        ...
        <div className='wrapperTopEventsF'>

        </div>
        <div className='flex items-center justify-center w-full p-[20px] text-xl font-bold text-[white]'><h1 >FESTIVALES</h1></div>
        <div className='flex items-center justify-center w-full'> <div className='flex  w-[95%] p-[5px]'><h1 className='text-[white]'>Liste de festivales :</h1></div></div>
            ...
            <div className='wrapperEvents'>
              {filteList.map((item, index) => (
                <div key={index} onClick={() =>{
                  navigate("/pay",{state:{cover:item.cover,title:item.title,lieu:item.lieu,prix:item.prix,date:item.date,heure:item.heure,event:"Festival"}})
                }}  className='items'>
                      <div className='wrapperImageEvent'>
                         <img className='imageEvent' src={item.cover} alt="" />
                    </div>
                    <div className='buttonEvent'>
                       <div className='flex w-[85%] pl-[20px] h-full flex-col justify-evenly bg-[white] rounded-bl-[15px]'>
